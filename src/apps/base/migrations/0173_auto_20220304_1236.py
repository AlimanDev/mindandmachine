# Generated by Django 3.2.9 on 2022-03-04 12:36

from decimal import Decimal
from django.db import migrations, models
from django.conf import settings

def set_fiscal_defaults(apps, schema_editor):
    fiscal_sheet_divider_alias = getattr(settings, 'FISCAL_SHEET_DIVIDER_ALIAS', None)
    timesheet_max_hours_threshold = getattr(settings, 'TIMESHEET_MAX_HOURS_THRESHOLD', Decimal('12.00'))
    timesheet_min_hours_threshold = getattr(settings, 'TIMESHEET_MIN_HOURS_THRESHOLD', '4.00')
    timesheet_divider_sawh_hours_key = getattr(settings, 'TIMESHEET_DIVIDER_SAWH_HOURS_KEY', 'curr_month')
    if callable(timesheet_min_hours_threshold):
        timesheet_min_hours_threshold = 'default_get_min_threshold'
    else:
        timesheet_min_hours_threshold = str(timesheet_min_hours_threshold)

    Network = apps.get_model('base', 'network')
    Network.objects.update(
        fiscal_sheet_divider_alias=fiscal_sheet_divider_alias,
        timesheet_max_hours_threshold=timesheet_max_hours_threshold,
        timesheet_min_hours_threshold=timesheet_min_hours_threshold,
        timesheet_divider_sawh_hours_key=timesheet_divider_sawh_hours_key,
    )

class Migration(migrations.Migration):

    dependencies = [
        ('base', '0172_merge_20220225_1151'),
    ]

    operations = [
        migrations.AddField(
            model_name='network',
            name='fiscal_sheet_divider_alias',
            field=models.CharField(blank=True, choices=[('nahodka', 'Находка'), ('pobeda', 'Победа'), ('shift_schedule', 'По расписанию смен')], help_text='Если не указано, то при расчете табеля разделение на осн. и доп. не производится', max_length=64, null=True, verbose_name='Алгоритм разделения табеля'),
        ),
        migrations.AddField(
            model_name='network',
            name='timesheet_divider_sawh_hours_key',
            field=models.CharField(default='curr_month', max_length=128),
        ),
        migrations.AddField(
            model_name='network',
            name='timesheet_max_hours_threshold',
            field=models.DecimalField(decimal_places=2, default=Decimal('12.00'), max_digits=5, verbose_name='Максимальное количество часов в белом табеле'),
        ),
        migrations.AddField(
            model_name='network',
            name='timesheet_min_hours_threshold',
            field=models.CharField(default='4.00', help_text='Может принимать либо числовое значение, либо название функции', max_length=64, verbose_name='Минимальное количество часов в белом табеле'),
        ),
        ...
        migrations.RunPython(set_fiscal_defaults, migrations.RunPython.noop),
    ]
