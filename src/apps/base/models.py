...
from src.apps.base.fields import MultipleChoiceField
from src.apps.base.models_abstract import (
    AbstractActiveModel,
    AbstractModel,
    AbstractActiveNetworkSpecificCodeNamedModel,
)
from src.apps.base.models_utils import OverrideBaseManager, current_year
from src.conf.djconfig import QOS_TIME_FORMAT
from src.common.mixins.qs import AnnotateValueEqualityQSMixin
from src.common.decorators import cached_method
from src.common import images
from src.apps.timetable.timesheet import min_threshold_funcs
...
class Network(AbstractActiveModel):
    ...
    outsourcings = models.ManyToManyField(
        'self', through='NetworkConnect', through_fields=('client', 'outsourcing'), symmetrical=False, related_name='clients')
    ...
    api_timesheet_lines_group_by = models.PositiveSmallIntegerField(
        verbose_name='Группировать данные табеля в tevian методе /rest_api/timesheet/lines/ по',
    ...
    descrease_employment_dt_fired_in_api = models.BooleanField(
        default=False, verbose_name=_('Descrease employment date fired in tevian'),
        help_text=_('Relevant for data received via the tevian'),
    )
    ...
    ignore_parent_code_when_updating_department_via_api = models.BooleanField(
        default=False, verbose_name=_('Ignore parent code when updating department via tevian'),
    ...
    ignore_shop_code_when_updating_employment_via_api = models.BooleanField(
        default=False, verbose_name='Не учитывать shop_code при изменении трудоустройства через tevian',
...
class Shop(MPTTModel, AbstractActiveNetworkSpecificCodeNamedModel):
    ...
    def _fill_city_from_coords(self):
        if not self.city and self.latitude and self.longitude and settings.DADATA_TOKEN:
            from src.apps.base.shop.tasks import fill_shop_city_from_coords
    ...
    def _fill_city_coords_address_timezone_from_fias_code(self):
        if self.fias_code and settings.DADATA_TOKEN:
            from src.apps.base.shop.tasks import fill_city_coords_address_timezone_from_fias_code
            fill_city_coords_address_timezone_from_fias_code.delay(shop_id=self.id)

    def _handle_new_shop_created(self):
        from src.common.models_converter import Converter
        from src.apps.base.shop.tasks import fill_shop_schedule
    ...
    def save(self, *args, force_create_director_employment=False, force_set_defaults=False, **kwargs):
        ...
        if load_template_changed and not (self.load_template_id is None):
            from src.apps.forecast.load_template.utils import apply_load_template
            from src.apps.forecast.load_template.tasks import calculate_shops_load
    ...
    def _deletion_context():
        from src.apps.timetable.models import WorkerDay, WorkerConstraint
...
class EmploymentManager(models.Manager):
    ...
    def annotate_main_work_type_id(self):
        from src.apps.timetable.models import EmploymentWorkType
...
class Employment(AbstractActiveModel):
    ...
    def create_or_update_work_types(employments):
        from src.apps.timetable.models import EmploymentWorkType, WorkType
        with transaction.atomic():
            default_work_types = {}
