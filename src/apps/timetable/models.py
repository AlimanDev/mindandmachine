...
from src.apps.base.models import Shop, Employment, User, Network, Break, Employee, Group
...
from src.adapters.mda.tmp_backport import ArraySubquery
from src.apps.recognition.events import EMPLOYEE_WORKING_NOT_ACCORDING_TO_PLAN
from src.apps.tasks.models import Task
from src.apps.timetable.break_time_subtractor import break_time_subtractor_map
from src.apps.timetable.exceptions import (
...
from src.common.commons import obj_deep_get
from src.common.mixins.qs import AnnotateValueEqualityQSMixin
...
class WorkType(AbstractActiveModel):
    ...
    def save(self, *args, **kwargs):
        from src.apps.forecast.models import OperationType
...
class WorkerDay(AbstractModel):
    ...
    у него должен быть указан магазин (shop).
    """
    class Meta:
    ...
    def _check_delete_qs_perm(cls, user, delete_qs, **kwargs):
        from src.apps.timetable.worker_day_permissions.checkers import DeleteQsWdPermissionChecker
    ...
    def _approve_delete_scope_filters_wdays(cls, **kwargs):
        ...
        from src.interfaces.api.serializers.worker_day import WorkerDayApproveSerializer
    ...
    def _check_create_single_obj_perm(cls, user, obj_data, check_active_empl=True, **extra_kwargs):
        from src.apps.timetable.worker_day_permissions.checkers import CreateSingleWdPermissionChecker
    ...
    def _check_update_single_obj_perm(cls, user, existing_obj, obj_data, check_active_empl=True, **extra_kwargs):
        from src.apps.timetable.worker_day_permissions.checkers import (
    ...
    def _check_delete_single_obj_perm(cls, user, existing_obj=None, obj_id=None, check_active_empl=False, **extra_kwargs):
        from src.apps.timetable.worker_day_permissions.checkers import DeleteSingleWdPermissionChecker
    ...
    def _check_delete_single_wd_data_perm(cls, user, obj_data, check_active_empl=False, **extra_kwargs):
        from src.apps.timetable.worker_day_permissions.checkers import DeleteSingleWdDataPermissionChecker
    ...
    def calc_day_and_night_work_hours(self, work_hours=None, work_start=None, work_end=None):
        from src.apps.timetable.worker_day.utils.utils import time_intersection
        from src.apps.timetable.worker_day.utils.xlsx_utils.tabel import time2hours
    ...
    def save(self, *args, recalc_fact=True, **kwargs): # todo: aa: частая модель для сохранения, отправлять запросы при сохранении накладно
        ...
        if settings.MDA_SEND_USER_TO_SHOP_REL_ON_WD_SAVE \
            ...
            from src.adapters.mda.tasks import create_mda_user_to_shop_relation
...
class AttendanceRecords(AbstractModel):
    ...
    def save(self, *args, recalc_fact_from_att_records=False, **kwargs):
        ...
        if self.type == self.TYPE_NO_TYPE:
            logger.info(f'Факт не сохраняется потому что нет типа {self.user.last_name} {self.user.first_name}')
            return res
            ...
            if self.type == self.TYPE_LEAVING and \
                ...
                if fact_approved:
                    ...
                    if fact_approved.last_edited_by_id and (
                        ...
                        logger.info(f'Факт не сохраняется потому что факт подтвержденный и '
                                    f'настройка "Сохранение руками" для {self.user.last_name} {self.user.first_name}')
                        return res
                    ...
                    if skip_condition:
                        logger.info(f'Факт не сохраняется потому что приход и {self.dttm} > '
                                    f'{fact_approved.dttm_work_start} для {self.user.last_name} {self.user.first_name}')
                        return res
                    ...
                    if self.type == self.TYPE_LEAVING:
                        ...
                        if prev_fa_wd:
                            ...
                            if prev_fa_wd.last_edited_by_id:
                                logger.info(f'Факт не сохраняется потому что уход и факт сохранен руками '
                                            f'для {self.user.last_name} {self.user.first_name}')
                                return res
                            ...
                            logger.info(f'Мы пересохраняем отметку с новой датой а также пересчитываем табель и создаем'
                                        f' неподтвержденный факт для {self.user.last_name} {self.user.first_name}')
                            return res

                        if self.shop.network.skip_leaving_tick:
                            logger.info(f'Пропуск отметки на уход для {self.user.last_name} {self.user.first_name}')
                            return res
