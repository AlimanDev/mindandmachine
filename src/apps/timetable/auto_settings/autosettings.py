...
from src.apps.base.models import Shop, Employment, ProductionDay, ShopSettings, WorkerPosition
from src.apps.forecast.models import PeriodClients
...
from src.apps.timetable.vacancy.tasks import create_shop_vacancies_and_notify, cancel_shop_vacancies
from src.apps.timetable.worker_day.stat import CalendarPaidDays, WorkersStatsGetter
...
class AutoSettings:
    '''Auto scheduling. Closely integegrated with algorithms (algo/ML/AI) project, over HTTP requests.
    Handles data preparation, timetable CRUD and all database interactions.'''
    #TODO: full refactoring, handling errors, returning verbose 400's etc.
    ...
    def create_timetable(self, form: dict) -> None:
        """
        Формат данных -- v1.3
        ...
        :return:
        """
        #TODO: eliminate flask layer in algo and launch jobs in algo_celery directly? Uses the same Redis anyway. Better job control, less "hanging" statuses.
        
        shop_id = form['shop_id']
        dt_from = form['dt_from']
        dt_to = form['dt_to']

        if not (self.network.get_acc_period_range(dt_to) == self.network.get_acc_period_range(dt_from)):
            raise ValidationError(self.error_messages["tt_different_acc_periods"])

        dt_min = datetime.now().date() + timedelta(days=self.network.rebuild_timetable_min_delta)

        if dt_from < dt_min:
            raise ValidationError(self.error_messages["tt_create_past"].format(num=self.network.rebuild_timetable_min_delta))
        ...
        try:
            ...
            self._fill_wd_array(worker_days_db, new_worker_days, prev_data=form.get('is_remaking', False))
            ...
            self._fill_wd_array(worker_days_db, prev_worker_days, prev_data=True)
            ...
            employment_stat_dict = self._count_prev_paid_days(dt_from - timedelta(days=1), employments, shop.region_id)
            # month_stat = _count_prev_paid_days(dt_to + timedelta(days=1), employments, shop.region_id, dt_start=dt_from, is_approved=not form['use_not_approved'])
            # month_stat_prev = _count_prev_paid_days(dt_from, employments, shop.region_id, dt_start=dt_first, is_approved=not form['use_not_approved'])
                ...
                'cashiers': [
                    {
                        ...
                        'is_dm': self._get_position_status(e),
            ...
            res = self.send_to_algo(data)
            tt.task_id = res.json().get('task_id', '')
            if not tt.task_id:
    ...
    def send_to_algo(cls, data: dict, api_route: str = '') -> requests.Response:
        try:
            ...
            raise ValidationError(cls.error_messages['tt_server_error'])       

    @staticmethod
    def _get_position_status(employment):
    ...
    @staticmethod
    def _fill_wd_array(worker_days_db, array, prev_data=False):
    ...
    @staticmethod
    def _count_prev_paid_days(dt_end, employments, region_id, dt_start=None, is_approved=True):
        """
    ...
    def set_timetable(self, form: dict) -> dict:
        try:
    ...
    def delete_timetable(self, form: dict) -> int:
        shop_id = form['shop_id']
        ...
        for tt in tts:
            if (tt.status == ShopMonthStat.PROCESSING) and (not tt.task_id is None):
                try:
                    self.send_to_algo({'id': tt.task_id}, 'delete_task')
