# Generated by Django 2.2.16 on 2021-05-18 21:52
...
def upd_v_mda_users(apps, schema_editor):
    ...
    WHERE True
      ...
      AND (fg.code IS NULL OR (fg.code::text <> ALL (ARRAY ['admin'::text, 'controller'::text])))
      AND ((e.dt_fired > '2020-10-01'::date AND e.dt_hired < e.dt_fired) OR e.dt_fired is null)
      AND e.dt_hired <= CURRENT_TIMESTAMP::date
      AND (NOT g.code = 'director' OR wp.group_id IS NULL OR
           g.code = 'director' AND (wdpa.type IS NULL OR (wdpa.type::text <> ALL (ARRAY ['M'::text]))))
    ;
""")
...
class Migration(migrations.Migration):
    dependencies = [
        ('integration', '0002_auto_20210118_1156'),
