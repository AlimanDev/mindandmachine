...
from src.interfaces.api.serializers.mda import (
    ShopDTOSerializer,
    RegionDTOSerializer,
...
class MdaIntegrationHelper:
    ...
    def export_data(self, threshold_seconds=None, plain_shops=False, export_path=None, output=None):
        ...
        """
        from src.apps.integration.mda.integration import MdaIntegrationHelper
    ...
    def sync_orgstruct(self, threshold_seconds=settings.MDA_SYNC_DEPARTMENTS_THRESHOLD_SECONDS):
        resp = requests.post(
            url=settings.MDA_PUBLIC_API_HOST + '/tevian/public/v1/mindandmachine/loadOrgstruct',
            json=self._get_orgstruct_data(threshold_seconds=threshold_seconds),
            headers={
                'x-public-token': settings.MDA_PUBLIC_API_AUTH_TOKEN,
            },
            timeout=settings.REQUESTS_TIMEOUTS['mda_sync_orgstruct']
        )
        try:
    ...
    def sync_users(self, threshold_seconds=settings.MDA_SYNC_DEPARTMENTS_THRESHOLD_SECONDS):
        resp = requests.post(
            url=settings.MDA_PUBLIC_API_HOST + '/tevian/public/v1/mindandmachine/loadUsers',
            json=self._get_users_data(threshold_seconds=threshold_seconds),
            headers={
                'x-public-token': settings.MDA_PUBLIC_API_AUTH_TOKEN,
            },
            timeout=settings.REQUESTS_TIMEOUTS['mda_sync_users']  # бешеный таймаут, т.к. запрос по всем юзерам может идти оооочень долго
        )
        try:
            resp.raise_for_status()
        except requests.RequestException:
