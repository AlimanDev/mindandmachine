...
def set_param_list(shop_id):
    ...
    for cashbox_type in CashboxType.objects.filter(shop_id=shop_id, do_forecast=CashboxType.FORECAST_HARD):
        if cashbox_type.is_main_type:
            params_dict[cashbox_type.id] = {
                'max_depth': 10,
                'eta': 0.2,
                'min_split_loss': 200,
                'reg_lambda': 2,
        ...
        elif cashbox_type.name == 'Доставка':
            params_dict[cashbox_type.id] = {
                'max_depth': 3,
                'eta': 0.1,
                'min_split_loss': 200,
...
# for celery
default_dt = (PeriodDemand.objects.all().order_by('dttm_forecast').last().dttm_forecast + timedelta(hours=7)).date()


def create_predbills_request_function(shop_id, dt=default_dt):
