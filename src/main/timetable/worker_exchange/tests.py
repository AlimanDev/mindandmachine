...
class TestWorkerExchange(LocalTestCase):
    ...
    def test_get_workers_to_exchange(self):
        ...
        response = self.api_get(
            '/api/timetable/worker_exchange/get_workers_to_exchange?specialization={work_type_id}&dttm_start={0}T09:00:00&dttm_end={0}T18:00:00'.format(
                self.qos_dt,
    ...
    def test_notify_workers_about_vacancy(self):
        ...
        response = self.api_post('/api/timetable/worker_exchange/notify_workers_about_vacancy',
                                 {'work_type': self.work_type2.id, 'dttm_start': '{}T09:00:00'.format(self.qos_dt),
                                  'dttm_end': '{}T15:00:00'.format(self.qos_dt)})
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()['code'], 200)

        response = self.api_post('/api/timetable/worker_exchange/notify_workers_about_vacancy',
                                 {'work_type': self.work_type2.id, 'dttm_start': '{}T15:00:00'.format(self.qos_dt),
