...
def select_cashiers(request, form):
    ...
    if len(work_types) > 0:
        employments_ids = EmploymentWorkType.objects.select_related('work_type').filter(
...
def get_cashier_info(request, form):
    ...
    if 'work_type_info' in form['info']:
        worker_work_type = EmploymentWorkType.objects.filter(employment=employment, is_active=True)
...
def set_worker_restrictions(request, form):
    ...
    curr_work_types = {wci.work_type_id: wci for wci in EmploymentWorkType.objects.filter(employment=employment, )}
    ...
    EmploymentWorkType.objects.filter(id__in=del_old_wcis_ids).delete()
