...
from src.apps.base.models import Employment, User, Shop, Employee, Network
from src.apps.base.models import NetworkConnect
from src.interfaces.api.serializers.base import BaseModelSerializer, BaseSerializer, ModelSerializerWithCreateOnlyFields, NetworkListSerializer, UserShorSerializer, NetworkSerializer
...
class WorkerDaySerializer(ModelSerializerWithCreateOnlyFields, UnaccountedOvertimeMixin):
    ...
    dttm_work_start = serializers.DateTimeField(default=None)
    dttm_work_end = serializers.DateTimeField(default=None)
    dt_not_actual = serializers.DateField(required=False, allow_null=True)
...
class ConfirmVacancyToWorkerSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        try:
            has_group_permission: bool = WorkerDay._has_group_permissions(user, vacancy.employee_id, vacancy.dt)
        except ValidationError:
            # Fired at the moment
            has_group_permission: bool = True

        no_perm_to_reconfirm = self.context['view'].action == 'reconfirm_vacancy_to_worker' and \
            not vacancy.employee.user_id == user.id and not has_group_permission
