...
from src.apps.base.models import Employment, Shop, Region, NetworkConnect
from src.apps.base.permissions import Permission
from src.interfaces.api.serializers.shop import SetLoadTemplateSerializer, ShopSerializer, ShopStatSerializer, serialize_shop
from src.apps.base.shop.utils import get_tree
from src.apps.base.views_abstract import UpdateorCreateViewSet
from src.common.openapi.responses import shop_tree_response_schema_dict as tree_response_schema_dict
...
class ShopViewSet(UpdateorCreateViewSet):
    ...
    def perform_create(self, serializer):
        serializer.save(region=Region.objects.first())  # TODO: переделать на получение региона по коду в tevian???
    ...
    def _get_tree(self, shops, user):
        ...
        now = datetime.datetime.now()
        for shop in shops:
            children = self.get_queryset().filter(parent_id=shop.id).filter(
                Q(dttm_deleted__isnull=True) | Q(dttm_deleted__gte=now),
                Q(dt_closed__isnull=True) |
                Q(dt_closed__gt=now.today() - datetime.timedelta(days=user.network.show_closed_shops_gap)),
            )
            result_children = self._get_tree(children, user)
            result.append({
                'id': shop.id,
                'label': shop.name,
