...
class TestEmploymentAPI(TestsHelperMixin, APITestCase):
    ...
    def test_batch_update_or_create_diff_report(self):
        ...
        attachment1 = open_workbook(file_contents=mail.outbox[0].attachments[0][1])
        df_created = pd.read_excel(attachment1, dtype=str, sheet_name='Создано', engine='xlrd')
        df_deleted = pd.read_excel(attachment1, dtype=str, sheet_name='Удалено', engine='xlrd')
        df_before_update = pd.read_excel(attachment1, dtype=str, sheet_name='До изменений', engine='xlrd')
        df_after_update = pd.read_excel(attachment1, dtype=str, sheet_name='После изменений', engine='xlrd')
        df_skipped = pd.read_excel(attachment1, dtype=str, sheet_name='Пропущено', engine='xlrd')
        ...
        attachment2 = open_workbook(file_contents=mail.outbox[1].attachments[0][1])
        df_created = pd.read_excel(attachment2, dtype=str, sheet_name='Создано', engine='xlrd')
        df_deleted = pd.read_excel(attachment2, dtype=str, sheet_name='Удалено', engine='xlrd')
        df_before_update = pd.read_excel(attachment2, dtype=str, sheet_name='До изменений', engine='xlrd')
        df_after_update = pd.read_excel(attachment2, dtype=str, sheet_name='После изменений', engine='xlrd')
        df_skipped = pd.read_excel(attachment2, dtype=str, sheet_name='Пропущено', engine='xlrd')
        ...
        attachment3 = open_workbook(file_contents=mail.outbox[2].attachments[0][1])
        df_created = pd.read_excel(attachment3, dtype=str, sheet_name='Создано', engine='xlrd')
        df_deleted = pd.read_excel(attachment3, dtype=str, sheet_name='Удалено', engine='xlrd')
        df_before_update = pd.read_excel(attachment3, dtype=str, sheet_name='До изменений', engine='xlrd')
        df_after_update = pd.read_excel(attachment3, dtype=str, sheet_name='После изменений', engine='xlrd')
        df_skipped = pd.read_excel(attachment3, dtype=str, sheet_name='Пропущено', engine='xlrd')
        self.assertEqual(len(df_created.index), 0)
        self.assertEqual(len(df_deleted.index), 0)
