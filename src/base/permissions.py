...
class WdPermission(Permission):
    def has_permission(self, request, view):
        ...
        if view_action in ['create', 'update']:
            ...
            ) and WorkerDay._has_group_permissions(request.user, request.data.get('employee_id'), request.data.get('dt'))
        elif view_action == 'destroy':
            wd_dict = WorkerDay.objects.filter(id=view.kwargs['pk']).values('type', 'dt', 'is_fact', 'employee_id').first()
            if not wd_dict:
                return False
            return GroupWorkerDayPermission.has_permission(
                user=request.user,
