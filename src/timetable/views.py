...
class WorkerDayViewSet(viewsets.ModelViewSet):
    ...
    def approve(self, request):
        ...
        for work_type in set(WorkerDayCashboxDetails.objects.filter(worker_day__in=wdays_to_approve).values_list('work_type_id', flat=True)):
            cancel_vacancies(serializer.data['shop_id'], work_type.id, approved=True)
