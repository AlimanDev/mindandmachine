...
def update_view_schedule_deviations(apps, schema_editor):
    schema_editor.execute("drop view timetable_schedule_deviations;")
    schema_editor.execute("""
    CREATE OR REPLACE VIEW timetable_schedule_deviations AS
 SELECT string_agg(wd.id::text, '-'::text ORDER BY wd.is_fact) AS id,
    wd.dt,
    wd.shop_id,
    employment.shop_id as employment_shop_id,
    employment.position_id,
    s.name AS shop_name,
    s.code AS shop_code,
    employment_shop.name AS employment_shop_name,
   ...
   FROM timetable_workerday wd
     ...
     LEFT JOIN base_employment employment ON wd.employee_id = employment.employee_id AND employment.id = (( SELECT max(be.id) AS max
     ...
     LEFT JOIN base_shop employment_shop ON employment.shop_id = employment_shop.id
     LEFT JOIN base_workerposition position ON employment.position_id = position.id
  WHERE wd.is_approved IS TRUE AND NOT (wd.employment_id IS NULL AND wd.type_id::text = 'W'::text AND wd.employee_id IS NOT NULL) AND NOT (employment IS NULL AND wd.employee_id IS NOT NULL)
  GROUP BY wd.dt, employee.user_id, employee.tabel_code, wd.type_id, u.username, 
  (concat(u.last_name, ' ', u.first_name, ' ', u.middle_name)), wd.shop_id, 
  s.name, s.code, (COALESCE(wd_details_wt_name.name, ''::character varying)), 
  wd.employee_id, shop_network.id, user_network.id, employment.shop_id, employment.position_id, 
  employment_shop.name, position.name;
    """)
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('timetable', '0110_merge_20211202_1035'),
    ]

    operations = [
        migrations.RunPython(update_view_schedule_deviations, migrations.RunPython.noop),
    ]
