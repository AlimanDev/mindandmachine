...
def replace_view_plan_and_fact_hours(apps, schema_editor):
    statement = "drop view metabase_financial_stat; drop view performance; drop view plan_and_fact_hours; drop view timetable_plan_and_fact_hours;"
    Network = apps.get_model('base', 'Network')
    if is_orteka := Network.objects.filter(code='orteka').exists():
