...
from src.base.shop.serializers import NonstandardShopScheduleSerializer, RestrictedTimeValidator
        ...
        def validate_time(data):
            for key, value in data.items():
                ...
                try:
                    ...
                    raise serializers.ValidationError(_('Invalid time format {time} for value {key}. The format must be {format}.').format(time=value, key=key, format=settings.QOS_TIME_FORMAT))
